...
ENEMIES_ROUTE = "/enemies/"


class TestEnemyRead:
    def test_read_enemies_list(self, client):
        response = client.get(ENEMIES_ROUTE)
    ...
    def test_read_enemy(self, client):
        response = client.get(f"{ENEMIES_ROUTE}1")
        assert response.status_code == 200
        enemy = response.json()
        assert enemy["id"] == 1
    ...
    def test_read_enemies_list_internal_error(self, client, monkeypatch):
        monkeypatch.setattr(AsyncSession, "scalars", async_failure)
        response = client.get(ENEMIES_ROUTE)
        assert response.status_code == 500
        assert response.json()["detail"] == ("Internal Server Error")

    def test_read_enemy_internal_error(self, client, monkeypatch):
        monkeypatch.setattr(AsyncSession, "get", async_failure)
        response = client.get(f"{ENEMIES_ROUTE}1")
        assert response.status_code == 500
        assert response.json()["detail"] == ("Internal Server Error")

    def test_read_enemy_not_found(self, client):
        response = client.get(f"{ENEMIES_ROUTE}999")
        assert response.status_code == 404
        error = response.json()
        assert error["detail"] == "Enemy not found"


class TestEnemyCreate:
    def test_create_enemy(self, client):
        request = {
        ...
        for key, value in test_enemy_sneak.items():
            assert enemy[key] == value

    def test_create_enemy_invalid(self, client):
    ...
    def test_create_enemy_internal_error(self, client, monkeypatch):
        monkeypatch.setattr(
            "api.routes.enemies.convert_to_db_enemy", sync_failure
        )
        ...
        response = client.post(ENEMIES_ROUTE, json=request)
        assert response.status_code == 500
        assert response.json()["detail"] == ("Internal Server Error")


class TestEnemyUpdate:
    def test_update_enemy(self, client):
        request = {
    ...
    def test_update_enemy_not_found(self, client):
    ...
    def test_update_enemy_internal_error(self, client, monkeypatch):
        monkeypatch.setattr(
            "api.routes.enemies.build_attack_list", sync_failure
        )

        request = {"actions": {"attacks": ["Longsword"], "spells": {}}}
        response = client.patch(f"{ENEMIES_ROUTE}1", json=request)
        assert response.status_code == 500
        assert response.json()["detail"] == ("Internal Server Error")


class TestEnemyDelete:
    def test_delete_enemy(self, client):
        response = client.delete(f"{ENEMIES_ROUTE}1")
        assert response.status_code == 200
    ...
    def test_delete_enemy_internal_error(self, client, monkeypatch):
        monkeypatch.setattr(AsyncSession, "delete", async_failure)
        response = client.delete(f"{ENEMIES_ROUTE}2")
        assert response.status_code == 500
        assert response.json()["detail"] == ("Internal Server Error")

    def test_delete_enemy_not_found(self, client):
        response = client.delete(f"{ENEMIES_ROUTE}999")
        assert response.status_code == 404
        error = response.json()
        assert error["detail"] == "Enemy not found"
